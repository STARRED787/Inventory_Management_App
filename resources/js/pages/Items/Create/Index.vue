...
<template>
    <AppLayout>
        <div class="flex h-full flex-1 flex-col gap-4 rounded-xl p-4">
            <h2 class="text-xl font-semibold">Welcome to Create Items</h2>

            <!-- Success -->
            <div
                v-if="flash.success"
                class="rounded bg-green-100 p-3 text-green-700"
            >
                {{ flash.success }}
            </div>

            <!-- Error -->
            <div v-if="flash.error" class="rounded bg-red-100 p-3 text-red-700">
                {{ flash.error }}
            </div>
            ...
            >
                ...
                <div>
                    ...
                    <select
                        v-model="form.unit"
                        class="w-full rounded border bg-gray-600 p-2"
                    >
